...
describe('Lottery Contract', ()=> {
    ...
    it('allows multiple account to enter', async() => {
        await lottery.methods.enter().send({
            from: accounts[0], //specifies which account
            value: web3.utils.toWei('0.02', 'ether')//converts eth to Wei
        });
        await lottery.methods.enter().send({
            from: accounts[1], //specifies which account
            value: web3.utils.toWei('0.02', 'ether')//converts eth to Wei
        });
        await lottery.methods.enter().send({
            from: accounts[2], //specifies which account
            value: web3.utils.toWei('0.02', 'ether')//converts eth to Wei
        });
...
try{ //for error handling with async
        await lottery.methods.enter().send({
            from: accounts[0],
            value: 200
        });
